...
export default function PricingManagerPage() {
  ...
  // Event Handlers
  const handleMarkupChange = async (id: string, newMarkup: number) => {
    try {
      await adminProductService.updateProductPricing(id, newMarkup);
      
      // Update local state
      setPrices((prev) =>
        prev.map((p) => {
          if (p.id === id) {
            const newRetail = calculateRetailPrice(p.price, newMarkup);
            const newMargin = calculateMargin(p.price, newRetail);
            return {
              ...p,
              markup: newMarkup,
  ...
  const handleStatusChange = async (id: string, newStatus: PriceStatus) => {
    try {
  ...
  const handleBulkUpdate = async () => {
    ...
    if (!isNaN(markup) && markup >= 0) {
      try {
        await adminProductService.bulkUpdatePricing(selectedPrices, markup);
        
        // Update local state
        setPrices((prev) =>
          prev.map((p) => {
            if (selectedPrices.includes(p.id)) {
              const newRetail = calculateRetailPrice(p.price, markup);
              return {
                ...p,
                markup,
                retailPrice: Math.round(newRetail),
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredPrices.map((price) => (
                    <tr key={price.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          <div className="text-sm text-gray-900 dark:text-gray-100">{price.merchant.businessName}</div>
                          <div className="text-xs text-gray-500 dark:text-gray-400">{price.merchant.id}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="bg-green-50 dark:bg-green-900/20 px-3 py-1.5 rounded border border-green-200 dark:border-green-800 inline-block">
                          <span className="text-sm font-bold text-green-700 dark:text-green-400">
                            ₦{(price.retailPrice || calculateRetailPrice(price.price, price.markup)).toLocaleString()}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-semibold text-blue-600 dark:text-blue-400">
                            {calculateMargin(price.price, price.retailPrice || calculateRetailPrice(price.price, price.markup || 0)).toFixed(2)}%
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
